...
export async function patchPost(
  ...
  const { data } = await axios.patch<PostType>(
    `https://jsonplaceholder.typicode.com/posts/${postId}`,
...
export async function postComment(
  ...
  const comment = await axios.post<CommentType>(
    `https://jsonplaceholder.typicode.com/posts/${postId}/comments`,
    data
  );
  return comment;
...
export async function postPost(data: PostType) {
  const post = await axios.post<PostType>(
    `https://jsonplaceholder.typicode.com/posts/`,
    data
  );
  return post;
...
export async function postUser(data: UserType) {
  const user = await axios.post<UserType>(
    `https://jsonplaceholder.typicode.com/users/`,
    data
  );
  return user;
...
export async function deleteComment(postId: number, commentId: number) {
  await axios.delete(
    `https://jsonplaceholder.typicode.com/posts/${postId}/comments?id=${commentId}`
  );
}
